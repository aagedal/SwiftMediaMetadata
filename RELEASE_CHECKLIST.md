...
- [ ] Review all changes intended for 2.0 and confirm the worktree is clean.
- [ ] Replace `Unreleased` in the 2.0.0 changelog heading with the release date.
- [ ] Confirm `Sources/CLI/SwiftMediaMetadata.swift` reports `2.0.0`.
- [ ] Run `Scripts/verify-release.sh 2.0.0` on macOS. It runs library and CLI
...
- [ ] Review the 1.x migration note and supported-format table in `README.md`.
- [ ] Commit the final release metadata.

## Publish

- [ ] Create an annotated `2.0.0` tag on the reviewed release commit.
...
- [ ] Confirm the GitHub release is public and its changelog-derived notes
  accurately describe the breaking module/package rename.
- [ ] Download the published archive on a clean arm64 Mac and run:
...
- [ ] Update `aagedal/homebrew-tap` to version 2.0.0 using the published archive
...
- [ ] Test a clean `brew install aagedal/tap/swift-exif`, `swift-exif --version`,
  and the Oslo geocode smoke command.
- [ ] Replace the README's pre-release `branch: "main"` SwiftPM dependency with
...
- [ ] Resolve a throwaway SwiftPM package against `from: "2.0.0"` and compile
  `import SwiftMediaMetadata`.
- [ ] Confirm the release page, checksum link, README links, and changelog compare
  link all resolve against the renamed repository.
- [ ] Record any deferred hardening/API-maintainability work in
