...
final class AIFFWriterTests: XCTestCase {
    ...
    func testFileWritePreservesExistingModificationDate() throws {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("test_\(UUID()).aiff")
        defer { try? FileManager.default.removeItem(at: fileURL) }

        let aiff = makeAIFFWithComm(channels: 2, sampleFrames: 100,
                                    sampleSize: 16, sampleRate: 48_000)
        try aiff.write(to: fileURL)
        let originalDate = Date(timeIntervalSince1970: 1_577_930_645.75)
        try FileManager.default.setAttributes(
            [.modificationDate: originalDate],
            ofItemAtPath: fileURL.path
        )

        var metadata = try AudioMetadata.read(from: fileURL)
