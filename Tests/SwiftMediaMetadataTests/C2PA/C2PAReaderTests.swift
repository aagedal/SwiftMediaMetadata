...
final class C2PAReaderTests: XCTestCase {
    ...
    func testParseManifestStore() throws {
        ...
        XCTAssertEqual(manifest?.claim.claimGenerator, "SwiftMediaMetadata Test")
    ...
    func testParseClaimV2WithGeneratorInfo() throws {
        let jumbfData = buildManifestStoreV2(name: "SwiftMediaMetadata", version: "2.0")
        let c2pa = try C2PAReader.parseManifestStore(from: jumbfData)
        let claim = c2pa?.activeManifest?.claim

        XCTAssertNotNil(claim?.claimGeneratorInfo)
        XCTAssertEqual(claim?.claimGeneratorInfo?.name, "SwiftMediaMetadata")
        XCTAssertEqual(claim?.claimGeneratorInfo?.version, "2.0")
        XCTAssertEqual(claim?.claimGenerator, "SwiftMediaMetadata 2.0")
    }
    ...
    private func buildMinimalManifestStore(claimGenerator: String = "SwiftMediaMetadata Test") -> Data {
