...
final class XMPSidecarTests: XCTestCase {
    ...
    func testWritePreservesExistingModificationDate() throws {
        var xmp = XMPData()
        xmp.headline = "Original"
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("test_\(UUID()).xmp")
        defer { try? FileManager.default.removeItem(at: tempURL) }

        try XMPSidecar.write(xmp, to: tempURL)
        let originalDate = Date(timeIntervalSince1970: 1_577_930_645.5)
        try FileManager.default.setAttributes(
            [.modificationDate: originalDate],
            ofItemAtPath: tempURL.path
        )

        xmp.headline = "Updated"
        let options = ImageMetadata.WriteOptions(fileModificationDate: .preserveExisting)
        try XMPSidecar.write(xmp, to: tempURL, options: options)
