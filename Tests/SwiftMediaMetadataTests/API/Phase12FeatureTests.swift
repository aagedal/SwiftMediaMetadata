...
final class Phase12FeatureTests: XCTestCase {
    ...
    func testRawFormatEnumCases() {
        ...
        XCTAssertTrue(allCases.contains(.gpr))
        XCTAssertEqual(allCases.count, 18) // 12 originals + iiq, 3fr, fff, x3f, mrw (Phase 24) + gpr
    }
    ...
    func testVideoFileWritePreservesExistingModificationDate() throws {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("test_\(UUID()).mp4")
        defer { try? FileManager.default.removeItem(at: fileURL) }

        try buildMP4WithMetadata(title: "Original", artist: nil).write(to: fileURL)
        let originalDate = Date(timeIntervalSince1970: 1_577_930_645.875)
        try FileManager.default.setAttributes(
            [.modificationDate: originalDate],
            ofItemAtPath: fileURL.path
        )

        var metadata = try VideoMetadata.read(from: fileURL)
