...
final class SafeWriteTests: XCTestCase {
    ...
    func testAtomicWritePreservesExistingModificationDate() throws {
        let tempDir = try createTempDir()
        defer { try? FileManager.default.removeItem(at: tempDir) }

        let fileURL = tempDir.appendingPathComponent("preserve-atomic.jpg")
        try makeMinimalJPEG().write(to: fileURL)
        let originalDate = Date(timeIntervalSince1970: 1_577_930_645.125)
        try setModificationDate(originalDate, for: fileURL)

        var metadata = try ImageMetadata.read(from: fileURL)
        metadata.iptc.headline = "Preserved"
        let options = ImageMetadata.WriteOptions(fileModificationDate: .preserveExisting)
        try metadata.write(to: fileURL, options: options)

        XCTAssertEqual(try modificationDate(for: fileURL).timeIntervalSince1970,
                       originalDate.timeIntervalSince1970,
                       accuracy: 0.001)
    }

    func testDefaultWriteUpdatesModificationDate() throws {
        let tempDir = try createTempDir()
        defer { try? FileManager.default.removeItem(at: tempDir) }

        let fileURL = tempDir.appendingPathComponent("update-date.jpg")
        try makeMinimalJPEG().write(to: fileURL)
        let oldDate = Date(timeIntervalSince1970: 1_577_930_645)
        try setModificationDate(oldDate, for: fileURL)

        var metadata = try ImageMetadata.read(from: fileURL)
        metadata.iptc.headline = "Updated"
    ...
    func testNonAtomicWritePreservesExistingModificationDate() throws {
        ...
        let fileURL = tempDir.appendingPathComponent("preserve-direct.jpg")
        try makeMinimalJPEG().write(to: fileURL)
    ...
    func testWriteToNewFileAssignsExplicitModificationDate() throws {
        let tempDir = try createTempDir()
        defer { try? FileManager.default.removeItem(at: tempDir) }

        let sourceURL = tempDir.appendingPathComponent("source-date.jpg")
        try makeMinimalJPEG().write(to: sourceURL)
        let metadata = try ImageMetadata.read(from: sourceURL)
        let destinationURL = tempDir.appendingPathComponent("destination-date.jpg")
        let sourceDate = Date(timeIntervalSince1970: 1_609_459_200.5)
        let options = ImageMetadata.WriteOptions(fileModificationDate: .set(sourceDate))

        try metadata.write(to: destinationURL, options: options)

        XCTAssertEqual(try modificationDate(for: destinationURL).timeIntervalSince1970,
                       sourceDate.timeIntervalSince1970,
                       accuracy: 0.001)
    }

    func testBatchProcessorForwardsWriteOptions() throws {
        let tempDir = try createTempDir()
        defer { try? FileManager.default.removeItem(at: tempDir) }

        let fileURL = tempDir.appendingPathComponent("batch-date.jpg")
        try makeMinimalJPEG().write(to: fileURL)
        let originalDate = Date(timeIntervalSince1970: 1_640_995_200.75)
        try setModificationDate(originalDate, for: fileURL)
        let options = ImageMetadata.WriteOptions(fileModificationDate: .preserveExisting)

        try BatchProcessor.process(file: fileURL, writeOptions: options) { metadata in
            metadata.iptc.headline = "Batch"
        }
