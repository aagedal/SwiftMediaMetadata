...
public struct ISOBMFFMetadata: Sendable {
    ...
    /// property a prior SwiftMediaMetadata version wrote) is removed.
    public static func updateMetadata(in boxes: inout [ISOBMFFBox], exif: ExifData?, xmp: XMPData?) throws {
        ...
        guard let metaIndex = boxes.firstIndex(where: { $0.type == "meta" }) else {
            ...
            // to describe, so no `iref` — SwiftMediaMetadata still reads the items back.
    ...
    private static func buildMetaBox(fromMeta: Data?, shiftThreshold: Int,
        ...
        // Items SwiftMediaMetadata owns: EXIF items and XMP `mime` items. These are
        ...
        // Strip any legacy `ipco` Exif/mime a prior SwiftMediaMetadata version appended,
    ...
    /// True for items SwiftMediaMetadata owns: EXIF items, or XMP `mime` items.
    ...
    /// Remove legacy `Exif` / XMP-`mime` properties that an older SwiftMediaMetadata
